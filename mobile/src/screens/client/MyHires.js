...
import ClientBrowseTalent from "./ClientBrowseTalent";
...
const MyHires = ({ navigation }) => {
  ...
  // const sampleImage = [
  //   "http://192.168.42.241/ehiremo/backend/uploads/feedback_photos/61d953e428ce13.98408624.jpg",
  //   "http://192.168.42.241/ehiremo/backend/uploads/feedback_photos/61d953e4294fe9.44851010.jpg",
  //   "http://192.168.42.241/ehiremo/backend/uploads/feedback_photos/61d953e42cdb58.26595499.jpg",
  // ]
  ...
  const loadData = () => {
    ...
    fetch("http://192.168.42.241/ehiremo/backend/api/appointments/client_fetch_my_appointments.php", {
      method: "POST",
        ...
        for (let row of result) {
          ...
          fetch(
            ...
            .then((responseJson2) => {
              //Hide Loader
              const result2 = JSON.parse(responseJson2);
              job_headline = result2.job_headline;
              // console.log(result2);
    ...
    //FREELANCER COUNT
    fetch('http://192.168.42.241/ehiremo/backend/api/users/hired_freelancer_count.php', {
      method: 'GET',
      headers: {
        //Header Defination
        'Accept': 'application/json',
  ...
  const goCancel = () => {
    ...
    fetch(
      "http://192.168.42.241/ehiremo/backend/api/appointments/cancel_appointment.php",
      {
        method: "PUT",
      ...
      .then((responseJson2) => {
        ...
        if (result2.message === "updated") {
          ...
          setModalAlert(true);
          setAlertText("Request has been sent to your freelancer.");
    ...
    >
      <View style={{
        ...
        <Avatar.Image
          ...
          size={55}
          source={{
            uri: row.freelancer_photo || sampleProfile
          }}
        />
        <View style={{ marginLeft: 15, flexDirection: "column" }}>
          ...
          <Text style={styles.caption2}>{row.freelancer_address}</Text>
        ...
        >
          ...
          {row.c_status === "cancel" ? (<>
            ...
            <TouchableOpacity
              ...
              onPress={() => cancelAppointment(row.id, row.freelancer_id, row.job_headline)}
            >
              <Text
                style={{
                  fontSize: 16,
      ...
      <View style={{ flexDirection: "column" }}>
        {row.f_status === "cancel" ? (<>
          <Text style={{ ...styles.caption2, color: "#1d4354", textDecorationLine: 'underline' }}>
            {row.freelancer_fname} wants to cancel this appointment.
          </Text>
        </>) : (<></>)}
        {row.f_status === "done" ? (<>
          <Text style={{ ...styles.caption2, color: "#14a800", textDecorationLine: 'underline' }}>
            {row.freelancer_fname} wants to mark this appointment done.
          </Text>
        </>) : (<></>)}
        ...
        <Text style={{ ...styles.caption2, color: "#555", marginTop: 5 }}>
          Job Headline : {row.job_headline}
        </Text>
        <Text style={{ ...styles.caption2, color: "#555", marginTop: 5 }}>
          Job Description : {row.proj_desc}
        </Text>
        ...
        <View style={{
          ...
          {row.c_status === "done" ? (<>
            ...
            <TouchableOpacity
              ...
              // onPress={() => inviteFreelancer(row.user_id)}
              onPress={() => markAsDone(row.id, row.freelancer_id, row.job_headline)}
            >
              <Text
                style={{
                  fontSize: 16,
  ...
  const goSendFeedback = () => {
    ...
    for (let row of sampleImage) {
      ...
      uploadData.append('file', { type: 'image/jpg', uri: row, name: 'uploadimagetmp.jpg' })

      fetch('http://192.168.42.241/ehiremo/backend/api/mobile_feedback_photos_upload.php', {
        method: 'POST',
        body: uploadData
      }).then(response => response.text())
        .then(responseJson => {
          ...
          fetch('http://192.168.42.241/ehiremo/backend/api/appointments/create_fb_appoint.php', {
            method: 'POST',
            body: JSON.stringify(fb_appoint),
          }).then(response => response.text())
            .then(responseJson => {
              const result = JSON.parse(responseJson);
              // setSampleImage([]);
    ...
    fetch('http://192.168.42.241/ehiremo/backend/api/appointments/create_feedback.php', {
      method: 'POST',
    ...
    fetch('http://192.168.42.241/ehiremo/backend/api/appointments/markasdone_appointment.php', {
      method: 'PUT',
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.screen}>
        <ScrollView showsVerticalScrollIndicator={false} refreshControl={
          ...
          >
            ...
            >{freelancerCount} Hired Freelancers found.</Text>
          </View>
      ...
      >
        <View style={styles.centeredView}>
          <View style={{ ...styles.modalView, height: 460 }}>
            ...
            <View style={{ textAlign: 'left', width: '100%' }}>
              ...
              <View style={{
                ...
                {sampleImage.length !== 0 && sampleImage.map((item, index) => (
                  <Image
                    key={index}
                    style={{ width: 90, height: 90, display: 'flex', flexDirection: 'row' }}
                    // resizeMode="cover"
                    source={{ uri: item }}
                  />
                ))}
