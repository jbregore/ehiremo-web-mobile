...
const ClientMyProfile = ({ navigation }) => {
  ...
  const [expandSource, setExpandSource] = useState("https://media.istockphoto.com/vectors/no-image-vector-symbol-missing-available-icon-no-gallery-for-this-vector-id1128826884?k=6&m=1128826884&s=170667a&w=0&h=F6kUwTcsLXUojmGFxN2wApEKgjx63zcIshCSOmnfEFs=");
  ...
  const loadData = () => {
    fetch('http://192.168.42.241/ehiremo/backend/api/users/user_fetch_self.php', {
      ...
      .then((responseJson) => {
        ...
        fetch('http://192.168.42.241/ehiremo/backend/api/appointments/client_fetch_profile_appointments.php', {
          method: 'POST',
          ...
          .then((responseJson) => {
            ...
            for (let row of result) {
              ...
              //JOB POST INFO
              fetch('http://192.168.42.241/ehiremo/backend/api/jobs/fetch_edit_job.php?id=' + row.jobpost_id, {
                method: 'GET',
                headers: {
                  //Header Defination
                  'Accept': 'application/json',
  ...
  useEffect(() => {
    ...
    for (let row of appointmentList) {
      //FEEDBACKS INFO
      fetch('http://192.168.42.241/ehiremo/backend/api/appointments/fetch_feedbacks.php?id=' + row.id, {
        method: 'GET',
        headers: {
          //Header Defination
          'Accept': 'application/json',
    ...
    for (let row of appointmentList) {
      console.log(row.id)
      //FEEDBACKS PHOTOS
      fetch('http://192.168.42.241/ehiremo/backend/api/appointments/fetch_photos.php', {
        method: 'POST',
        ...
        <View style={{
          ...
          <View style={{ flexDirection: "row", marginBottom: 15 }} key={index}>
            ...
            <View style={{ marginLeft: 15, flexDirection: "column", marginTop: 10 }} >
              <Pressable onPress={() => viewProfile(item.profile_userid)}>
                <Text style={{ ...styles.caption2, textDecorationLine: 'underline', color: "#14a800" }}>
              ...
              <View style={{ flexDirection: "row" }}>
                <Text style={{
                  ...styles.caption, marginTop: 3, marginRight: 3, color: "#14a800",
                  fontSize: 18
                }}>{item.profile_rating.substring(0, 4)}</Text>
          ...
          <View style={{ flexDirection: "column", marginBottom: 15 }}>
            ...
            <Text style={styles.caption2}><Text style={{ fontWeight: "bold", color: "#1d4354" }}>Project budget: </Text>{item.proj_cost}</Text>
            <Text style={styles.caption2}><Text style={{ fontWeight: "bold", color: "#1d4354" }}>Project address: </Text>{item.proj_addr}</Text>
            <Text style={styles.caption2}><Text style={{ fontWeight: "bold", color: "#1d4354" }}>Start date: </Text>{item.start_date}</Text>
            <Text style={styles.caption2}><Text style={{ fontWeight: "bold", color: "#1d4354" }}>End date: </Text>{item.end_date}</Text>
          ...
          <View style={{ flexDirection: "column" }}>
            <Text style={styles.caption2}>
              <Text style={{ color: "#1d4354", fontWeight: 'bold' }}>
                Client: </Text>
              <Text style={{ color: "#14a800", textDecorationLine: 'underline' }}>
                {info.name}
              </Text>
            </Text>
            <Text style={styles.caption2}>
            ...
            <Text style={styles.caption2}><Text style={{ color: "#1d4354", fontWeight: 'bold' }}>Job Description: </Text>
              {item.proj_desc}
            </Text>
          ...
          <ScrollView onScroll={({ nativeEvent }) => imageChange(nativeEvent)}
            showsHorizontalScrollIndicator={false} pagingEnabled horizontal style={{ marginTop: 15 }} >
                ...
                return (
                  <View key={key}>
                    ...
                    <Pressable onPress={() => showImageFull(row.fb_photos)}>
                      <Image key={key} style={styles.wrap} resizeMode="cover" source={{ uri: row.fb_photos || sampleProfile }} />
                    </Pressable>
                  </View>
                )
              } else {
                ...
                return (
                  <View key={index}>
                    <Text style={{
                      ...
                      Feedback from {
                        row.fb_from === item.profile_userid ? item.profile_fname : info.fname
                      }{" "}:</Text>
                    ...
                    <View style={{ flexDirection: "row", marginBottom: 10 }}>
                      ...
                      <Text style={{
                        ...styles.caption, marginTop: 3, marginRight: 3, color: "#14a800",
                        fontSize: 18
                      }}>{row.fb_star.substring(0, 4)}</Text>
  ...
  const NoAppointment = () => {
    return (
      <View style={{
        ...
        borderRadius: 20, borderWidth: 1, borderColor: "#c3c3c3",
        alignItems: 'center', justifyContent: 'center', backgroundColor: "rgb(247,247,247)"
      }}>
        <Image
          style={{
            width: 100, height: 100,
          ...
          resizeMode="cover"
        />
        <Text style={{ ...styles.caption2, marginTop: 5 }}>No appointments yet.</Text>
      </View>
    )
  }
                ...
                <View style={{ marginLeft: 15, flexDirection: "column" }}>
                  <View style={{ flexDirection: "row" }}>
                    <Text style={{
                      ...styles.caption, marginTop: 3, marginRight: 3, color: "#14a800",
                      fontSize: 18
                    }}>{info.rating.substring(0, 4)}</Text>
                  ...
                  <Text style={{ ...styles.caption, marginTop: 8, fontSize: 20, lineHeight: 20 }}>{info.name}</Text>
                  <Text style={{ ...styles.caption, marginTop: 3 }}>{info.address}</Text>
                  <Text style={{ ...styles.caption, marginTop: 3 }}>{info.role}</Text>
                  ...
                  {/* <View style={{
                    ...
                    <View style={{ flexDirection: 'row' }}>
                      <FontAwesome name="envelope" size={14} color="#1d4354" style={{ marginTop: 6, marginRight: 3 }} />
                      <Text style={{
                        marginTop: 3, fontSize: 16, textDecorationColor: "#1d4354",
                        color: "#1d4354", textDecorationLine: 'underline'
                      }}>Send message</Text>
                    </View>

                    <View style={{ flexDirection: 'row', marginLeft: 10 }}>
                      <Ionicons name="document-text" size={16} color="#1d4354" style={{ marginTop: 4, marginRight: 3 }} />
                      <Text style={{
                        marginTop: 3, fontSize: 16, textDecorationColor: "#1d4354",
                        color: "#1d4354", textDecorationLine: 'underline',
                      }}>View portfolio</Text>
