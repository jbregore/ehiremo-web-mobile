...
const TalentSettings = ({ navigation }) => {
  ...
  useEffect(() => {
    let datas = [];
    // console.log(info.service_offer)
    const fetchData = () => {
      if (info.service_offer === undefined) {

      } else {
        let services = info.service_offer.split(',');
  ...
  const saveChanges = () => {
    ...
    if (Object.keys(portfolio).length === 0 && portfolio.constructor === Object) {
      ...
      fetch('http://192.168.42.241/ehiremo/backend/api/users/talent_getting_started_pic.php', {
        method: 'PUT',
      ...
      fetch('http://192.168.42.241/ehiremo/backend/api/mobile_portfolio_upload.php', {
        method: 'POST',
        body: uploadData
      }).then(response => response.text())
        .then(responseJson => {
  ...
  const changeProfile = async () => {
    ...
    if (!result.cancelled) {
      ...
      fetch('http://192.168.42.241/ehiremo/backend/api/mobile_profile_picture_upload.php', {
        method: 'POST',
        body: uploadData
      }).then(response => response.text())
        .then(responseJson => {
          ...
          fetch('http://192.168.42.241/ehiremo/backend/api/users/client_profile_pic.php', {
            method: 'PUT',
            body: JSON.stringify(user_data),
            headers: {
              //Header Defination
  ...
  const switchRole = () => {
    ...
    fetch('http://192.168.42.241/ehiremo/backend/api/users/switch_into_client.php', {
      method: 'GET',
      headers: {
        //Header Defination
        'Accept': 'application/json',
  ...
  const changePassword = () => {
    if (!oldPassword || !newPassword || !confPassword) {
      ...
      if (newPassword.length < 8) {
        ...
        fetch('http://192.168.42.241/ehiremo/backend/api/users/change_password.php', {
          method: 'POST',
